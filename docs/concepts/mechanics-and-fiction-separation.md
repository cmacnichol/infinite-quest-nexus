...
Story Direction is not a hidden mechanics result. It is typed authorial input
whose stated events are requested fiction for the current turn, subject to
world canon and corrected continuity, so normal action assessment is skipped.
It also skips event evaluation and independent semantic scene coverage while
preserving dormant RPG and event data. It still passes instruction-leak and
canon validation; forbidden mechanics or engine directives cause an actionable
error rather than being silently removed. Auto classification is retired.

Related decisions: [ADR 0005](../architecture/0005-typed-private-story-orchestration.md) and [Story-only campaign policy](../architecture/story-only-campaign-policy.md).
