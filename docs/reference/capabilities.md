...
- Campaign turn-control styles for Actions only, Action, and Story Direction
- Frozen policy provenance retained across retry, branch, and portable export
...
- Typed Action and Story Direction prompt contracts; Story Direction skips RPG,
  event, and independent scene-coverage stages while preserving dormant state
- Four distinct generated directions plus a bounded choice-only repair path
