...
Use protocol source-extraction-v3-quote-anchor. Raw provider facts contain subject/predicate/value/category and an exact quoted passage with its selected paragraph; the server derives canonical Unicode code-point coordinates from that quote inside the selected chunk, then assigns stable application IDs after validation. When a provider also supplies legacy coordinates, validate them strictly as a candidate span and quote; invalid coordinates are rejected rather than re-anchored, and valid coordinates can disambiguate repeated quotes. Bound each chunk to 200 facts and each fact value to 4,000 characters. If a chunk exceeds output/fact limits after repair, split and retry smaller spans rather than accepting a prefix. Count resulting subchunks against the 200-chunk job maximum.

**Evidenced refinement (P3.9 live diagnostics):** the v2 prompt/protocol produced one successful extraction and review path. Earlier live evidence included a coordinate failure, and one later post-v2 selected-provider closure reported `source_quote`; the exact mismatch cause remains unknown. The v3 quote-anchor contract keeps exact quote validation strict while deriving canonical coordinates server-side; valid legacy coordinates remain a strict compatibility and repeated-quote disambiguation input.
