...
**Goal:** Add an authoritative Story only campaign workflow that bypasses RPG/event checks, generates relevant narrative choices, and repairs only choices when possible; update both the served legacy UI and the new UI to use the existing Action / Story Direction setting and remove Auto.
...
This explicit user requirement takes precedence over general guidance that legacy UI parity is unnecessary. Apply it to the served legacy application files, not the reference-only root `index.html`. Include this requirement in both Terra task briefs and review handoffs so neither worker treats the other interface as optional or deferred.
...
| 7 | legacy UI, then new UI | 1–5 for isolated implementation; 6 for acceptance | Two fresh Terra implementers with separate surface reports; shared edits serialized |
...
Default to sequential implementers because Tasks 2–6 share large repository and runtime files. Parallelism is allowed for read-only reviews or test analysis and the explicit Task 7 handoff exception below; never dispatch two writers to the same file. A fresh agent per task provides isolation without requiring simultaneous implementation.

Task 6 closeout may assign a separate Terra test-only worker the new `tests/unit/portable-accepted-policy-metadata.test.ts` file for shared portable-policy boundary coverage. This worker owns no production files or existing fixtures and reports defects to the sole Task 6 production owner. The controller reviews and serializes its checkpoint with the complete Task 6 change.

Task 6 also updates the obsolete `flexible_auto` campaign fixture in `tests/e2e/quiet-leaf-story.e2e.test.ts` to the normalized `flexible_action` value. The compiled System Archive release tests run the normal build, whose contract check includes this fixture; deferring this one fixture change to Task 7 would block Task 6 verification. This does not complete the UI test or remove its existing Auto expectations: Task 7B must replace those assertions and verify the revised keyboard interaction in the browser.

One explicit independent preparation exception is Task 7H below: after Task 1 review, a Terra worker may build the new disposable browser-harness files alongside Tasks 2–6. It owns only those new test/harness files and their focused tests, never shared production code or the other implementer's fixtures. The controller serializes commits, reviews its own diff separately, and hands the result to 7A. UI implementation normally waits for Tasks 1–6; the infrastructure exception below permits isolated work without closing those gates.

**Infrastructure exception for Task 7A:** when Tasks 1–5 are accepted and Task 6's contracts are fixed, the controller may start the legacy UI implementation while Task 6 compatibility remediation awaits database availability or review. The UI worker owns only its served UI, client helpers and associated tests; it must not edit Task 6 contracts, repositories, runtime import code or fixtures. Task 6 production files remain frozen during review. This exception changes work order only: Task 6 still needs its actual PostgreSQL/Linux checks and accepted remediation review before its checkpoint, and Task 7A still needs rendered desktop/mobile evidence and its own review before acceptance. Do not count source edits or mocks as the missing runtime checks. Keep Task 7B shared-file work serialized after the 7A handoff.

## Baseline and preparation

**Reviewed shared-helper handoff exception for Task 7B:** after Tasks 1–6 are accepted, 7A may freeze and release its shared helper files before its full legacy browser acceptance. Once the controller verifies those tests and a separate reviewer accepts the narrow handoff, a fresh Terra 7B worker may implement the new UI in disjoint files while 7A completes legacy-only fixes and browser verification. 7A must not resume editing released shared files without an explicit ownership transfer. Serialize edits to the shared browser suite and use of the disposable runtime; 7B initially adds its scenarios in a separate test file and the controller integrates the suite after 7A releases it. Keep builds compatible, record which source snapshot each browser run exercised, and verify final cross-interface behavior after both changes are present. This is a work-order exception only: neither surface is accepted without its own complete rendered evidence and review, and Task 8 still waits for Task 7 acceptance.
...
- Review `packages/database/src/generation-repository.ts` and its retry tests: the existing requeue operation removes `automaticRepair`. Persist and verify the separate choice-repair allowance through the actual repository retry operation, retaining historical explicit-retry behavior. Add this repository to Task 5 ownership if its retry projection needs changing.
...
- [ ] Bind completed repair provenance to the actual `ProviderResult.preparedRequest`, including a permitted response-format fallback. Retain enough body/serialization identity to recompute its hash from the frozen repair policy, protected base, and provider configuration on resume; an arbitrary stored hash is insufficient. Preserve the pre-dispatch attempt fence. Add a real transport regression for response-format rejection followed by successful fallback, then lease reclaim and explicit retry with no new provider calls; reject body or serialization-variant tampering and preserve the original narrative response chain.
...
A separate fresh Terra PostgreSQL test worker may own only the new `tests/integration/story-only-portability.integration.test.ts` and its scratch report for actual ZIP export/import/re-export, version admission, provenance, dormant-state preservation and owner isolation. It must use the dedicated test configuration on port 15439 and leave production, existing fixtures/tests, System Archive, transfer and branch work with the primary. Communicate concrete failures and release source before final verification; all Task 6 gates still apply to the combined deliverable.

After the ZIP test worker releases its files, a fresh Terra copy-test worker may own only `tests/integration/story-only-campaign-copies.integration.test.ts` and its scratch report. Cover real branch/transfer policy provenance, dormant state, destination-default isolation, owner fences and preview fingerprint tampering. The primary retains all production and existing tests, including copy implementation. Use only the dedicated PostgreSQL configuration; report RED/GREEN and release the file before combined review.

After the copy-test worker releases ownership, a fresh Terra System repository test worker may own only `tests/integration/story-only-system-portability.integration.test.ts` and its scratch report. Exercise actual PostgreSQL export/import/re-export of policy provenance and dormant state, owner remapping, historical null and atomic rejection. This supplements, and does not replace, the primary worker's actual Linux System Archive ingress/roundtrip tests. The primary remains the sole production writer.

A separate Terra harness worker may prepare only the ignored `tmp/story-only-test/task6-system-linux/` directory and its scratch report while System fixes proceed. It must reuse the dedicated test PostgreSQL service, create per-file disposable databases, exclude credentials from the image context, preserve test exit status, and clean up only its own Docker resources. It releases the runner before the primary rebuilds and executes it against the final source; preparation is not Linux test evidence.
...
- [ ] Seed accepted turns with the real `model_metadata.generationPolicy` runtime snapshot as well as `turns.generation_policy`. Use the shared `portableAcceptedTurnModelMetadata(metadata, runtimePolicy)` projection for branch/transfer metadata and audit the System archive boundary for the same nested copy. Assert the destination runtime column is null, the metadata has no `generationPolicy` key or sentinel supplement/repair text, and the exact portable provenance remains. Cover locally executed, imported, and historical-null cases; source accepted metadata remains unchanged. Campaign ZIP already uses a model-metadata allowlist; retain and verify it. A null runtime column alone does not prove runtime snapshots were excluded.
...
**Owner:** fresh Terra harness implementer; requires Task 1 contracts. Own only `tests/helpers/story-only-runtime-fixture.ts`, `tests/helpers/story-only-synthetic-provider.ts`, `scripts/story-only-ui-runtime.ts`, `scripts/story-only-synthetic-provider.ts`, `scripts/story-only-runtime-signal-runner.ts`, `playwright.story-only-runtime.config.ts`, and new focused tests under `tests/unit/story-only-runtime-harness.test.ts` and `tests/unit/story-only-runtime-startup-cleanup.test.ts`. Do not edit shared production code, existing fixtures, or implement either UI in this preparation task.
...
- [ ] Verify scenario control through the actual Docker transport: enqueue distinctive synthetic content and length/failure responses, consume them from a separate container, and read the resulting operation counts from the provider. Control requests must not inflate generation counts. Preserve a localhost-only native default and explicitly enable the unpublished container-network listener for Docker. An empty or malformed scenario must fail instead of silently queuing the default response.
- [ ] Verify graceful teardown after readiness and cancellation during startup. Register lifecycle handling before allocating resources, make concurrent shutdown requests share one cleanup operation, and inspect the absence of each owned container/network/database afterward. A terminal shell exiting does not prove its Node child received a signal or cleaned up. Provide a controlled Windows shutdown channel where terminal signals are unreliable, and retain SIGINT/SIGTERM behavior with direct process-level evidence. Surface cleanup failures without declaring the run disposed.
- [ ] Exercise startup failure through the actual fixture catch path with mocked external boundaries: failed allocation must not trigger deletion, successful partial allocation must be cleaned up, and genuine cleanup failures must remain visible. For signal checks, target the process importing `story-only-ui-runtime.ts`, capture its command and process identity, and verify cleanup without a separate stdin-stop command. Signaling only the child application container does not satisfy the harness signal gate. A test-only Linux runner may bridge the dedicated database transport while preserving the strict target validation.
...
**Owners:** fresh Terra legacy-UI implementer (7A), then fresh Terra new-UI implementer (7B). **Depends on:** Tasks 1–6 for acceptance; the infrastructure exception permits isolated 7A implementation after Tasks 1–5 with Task 6 contracts fixed. The controller supplies the shared behavior contract to both. Assign shared helper edits to 7A first and review its handoff before 7B starts. 7B completes the cross-client cleanup gate after both surface changes are present.
...
- 7H supplies the shared disposable browser harness: `tests/helpers/story-only-runtime-fixture.ts`, `tests/helpers/story-only-synthetic-provider.ts`, `scripts/story-only-ui-runtime.ts`, and `playwright.story-only-runtime.config.ts`. 7A consumes the reviewed harness and hands its UI scenarios and evidence to 7B and Task 8; do not recreate the harness in either UI task. Reuse `playwright.quiet-leaf-runtime.config.ts` and `tests/e2e/quiet-leaf-runtime.e2e.test.ts` for existing same-campaign route checks. The ordinary Vite/mock Playwright setup is insufficient for this gate.
...
- [ ] Test a setting change with the same campaign ID and accepted turn number: start in Action with draft text, selected choices and a length override, then synchronize Story Direction and verify the effective mode changes while those values remain intact. Repeat in reverse. `story-player-model.ts` currently returns early for the same draft owner before observing a changed style; fix that behavior without adding the style to draft ownership and thereby clearing the draft. Verify both new presenters and submission-time campaign authority.
...
The sanitized 12-case quality corpus may be authored and independently reviewed after Tasks 1–6 while Task 7 browser verification continues. It consumes the accepted narrative and choice contracts, not UI or runtime changes, and has separate file ownership. This allows useful preparation during browser repair without relaxing any acceptance gate: composed execution, benchmarks, aggregate checks, live-provider evaluation, and Task 8 acceptance still wait for Task 7. Preparing the corpus is not evidence of live model quality.
...
- [x] Create 12 quality cases covering dialogue, explicit multi-beat direction, established mystery, absent character, completed goal, changed location, contradictory requested outcome versus world rule, ambiguous action, dormant event, custom suggestion duplication, Unicode duplicate, and choice-only repair. Each record has `id`, fiction authority, input, required/forbidden claims, and expected available next developments. Assertions on supplied/mock output are validator evidence only. The reviewed corpus is `tests/fixtures/story-only/quality-cases.json`; its live evaluation remains unrun.
...
- Mark the active-classification portion of `docs/architecture/0021-turn-input-intent-classification.md` superseded by the new policy and update `docs/architecture/index.md`. Preserve its historical rationale and historical job-provenance rules; do not rewrite old audit/review artifacts as if they described the new implementation.
