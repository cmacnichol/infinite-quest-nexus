...
Import assigns fresh destination IDs for campaign, world records created by the import, turns, Chronicle records, segments, and assets, then rewrites known relationships and image links. Re-importing the same content fingerprint with the same destination choice returns the completed import instead of duplicating it. Importing that archive into a different compatible destination is a separate new campaign.
