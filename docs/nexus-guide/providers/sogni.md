...
3. Paste the key, then refresh the model picker or enter an image model ID manually. Sogni's advertised inventory may not identify every image-capable model in a way Nexus can filter reliably.
...
Use **Provider default (safest)** for sensitive-content filtering unless there is a reviewed need to request another mode. Declare workflow override support only after verifying it in Sogni's current documentation. A request does not override Sogni account eligibility or provider policy, and some workflow routes may not support an override.

If **Selected workflow documents filter overrides** is not selected, Nexus rejects an explicit enabled or disabled filter request before submission. This guard prevents an unverified workflow argument from silently weakening the provider default.
