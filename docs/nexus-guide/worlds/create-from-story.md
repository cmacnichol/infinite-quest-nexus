...
Applying creates or updates only the selected editable draft after normal revision checks. It never publishes a version or starts a campaign. The accepted source prefix, its evidence, provenance, selected boundary, and roster evidence become a `sourceMaterial` appendix in the saved world content. Rejected candidates, raw provider responses, prompts, credentials, and operational job state do not.
