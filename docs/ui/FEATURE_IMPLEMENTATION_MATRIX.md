...
adapter. A generic watcher for non-generation jobs remains deferred.
...
| Select / load / resume campaign | `docs/nexus-guide/campaigns/select-and-load.md` | `GET /api/v1/campaigns/:campaignId/sync-status?since=<syncToken>` | Story Player boot + C6 campaign projection | Implemented and wired | Live API | Bounded `replace`/`unchanged` turn window; active-only pending job; sanitized recovery can represent completed work outside the window | `packages/contracts/src/client-api.ts`, `packages/client-core/src/campaign-store.ts` | Retain — reconcile active pending, then recovery, then storage hint; never infer authority from a cursor/token |
...
| Turns list (accepted-turn ledger) | `docs/concepts/campaigns-and-turns.md` | `GET /api/v1/campaigns/:campaignId/turns?before=<opaque-cursor>` | Story Player scene/history views + C6 campaign projection | Implemented and wired | Live API | Self-identifying, campaign-scoped bounded pages; deduplicate before merge and preserve exact authoritative duplicates/upgrades | `packages/contracts/src/client-api.ts`, `packages/client-core/src/campaign-store.ts` | Retain; initial page comes from sync, older history is explicitly paged |
| Runtime state (get/update trackers, scratchpad) | `0011-editable-campaign-runtime-state.md` | `GET/PATCH /api/v1/campaigns/:campaignId/state` | `editStateDialog` (4 tabs) | Implemented and wired | Live API | Optimistic concurrency (revision-based) per `0011-editable-campaign-runtime-state.md` — UI conflict-handling not independently verified | `server.ts:581-595` | Retain; make revision-conflict outcome explicit (don't silently overwrite) |
...
| Retry-latest (replace accepted turn) | ADR 0017 | `POST .../generations/retry-latest` | retry-latest control + C6 campaign projection | Implemented and wired | Live API | Staged: only overwrites accepted turn after new output validates; every public status/recovery/stream payload carries `replace_latest` plus the durable target UUID | `packages/contracts/src/client-api.ts`, `packages/client-core/src/campaign-store.ts` | Retain; reconcile a completed replacement even when its target/result lies outside the current window, then sync authoritatively |
| Watch generation job status | ADR 0025 | `GET .../generation-jobs/:jobId`, `GET .../stream` (SSE) | `GenerationWorkflow` + client-web browser source + C6 session reducer | Implemented and wired | Live API | SSE primary with bounded polling fallback; Q1 resolved: visible narration is emitted only as typed `GenerationEvent.narration`; one app-owned watcher reduces into a private-run-free public projection | `packages/client-core/src/generation/workflow.ts`, `packages/client-web/src/generation/`, `packages/client-core/src/campaign-store.ts` | Retain the headless workflow and session reducer; do not restore an app-owned EventSource/poll/timeout monitor |
| Get finished job result | — | `GET .../generation-jobs/:jobId/result` | active `GenerationRun.fetchResult()` | Implemented and wired | Live API | A temporarily unavailable result remains complete-but-loading; retry fetches the result only | `packages/client-core/src/generation/workflow.ts` | Retain; never restart generation or the watcher for this state |
| Retry / discard recoverable or failed job | `docs/player-guide/recovering-a-generation.md` | `POST .../retry`, `POST .../discard` | active `GenerationRun` actions | Implemented and wired | Live API | Commands are bound to the authoritative job/run | `packages/client-core/src/generation/workflow.ts` | Retain — treat `recoverable` as a distinct, human-actionable status |
| Cancel an actively-running generation job | generation integrity contract | `POST .../generation-jobs/:jobId/cancel` | active `GenerationRun.cancelGeneration()` | **Implemented and wired** | Live API | Durable server cancellation is distinct from locally detaching/aborting a watcher | `services/api/src/server.ts`, `packages/client-core/src/generation/workflow.ts` | Retain the explicit distinction; navigation must not cancel a server job |
| Resume generation after page refresh | `docs/reference/capabilities.md` "pending-job resume" | `GET .../sync-status`, result endpoint, pending-submission store | `GenerationWorkflow.resume()` + C6 campaign projection | Implemented and wired | Live API + localStorage | Server pending is authoritative; sanitized recovery handles recoverable/failed/completed states; local storage is only a replay hint after server paths | `packages/client-core/src/generation/workflow.ts`, `packages/client-core/src/campaign-store.ts` | Retain; completed out-of-window recovery fetches only its result and then syncs—never replays a submission |
...
| Provider `configuration` field redaction | AGENTS.md Security | Credential-like values redacted on provider reads | provider edit form | Implemented and wired | Live API | List, `PUT .../default`, and `PATCH` responses without a `configuration` body all redact credential-like keys; `POST`/`PATCH` responses echo back only the configuration the caller just submitted, unredacted, for write round-trips | `provider-service.ts:58-63,145,209`, `packages/domain/src/redaction.ts` | Retain; do not log configuration values unnecessarily |
...
| Legacy single turn illustration | — | `POST /turns/:turnId/illustrations`, `PUT .../illustration-asset` | orphaned legacy handlers; no rendered control reaches them | Backend available, UI absent | Live API | Q2 resolved: the route family remains live, but the current frontend emits only segmented-image actions | `server.ts:869-878`, `OPEN_QUESTIONS.md` Q2 resolution | Do not reproduce this vestigial interaction model; use segmented illustrations only |
