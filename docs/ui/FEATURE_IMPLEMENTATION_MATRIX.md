...
| Watch generation job status | ADR 0025 | `GET .../generation-jobs/:jobId`, `GET .../stream` (SSE) | `GenerationWorkflow` + client-web browser source | Implemented and wired | Live API | SSE primary with bounded polling fallback; Q1 resolved: visible narration is emitted only as typed `GenerationEvent.narration` | `packages/client-core/src/generation/workflow.ts`, `packages/client-web/src/generation/` | Retain the headless workflow; do not restore an app-owned EventSource/poll/timeout monitor |
...
| Resume pending generation after page refresh | `docs/reference/capabilities.md` "pending-job resume" | `GET .../sync-status`, pending-submission store | `GenerationWorkflow.resume()` | Implemented and wired | Live API + localStorage | Server snapshot is authoritative; local storage is only a replay hint | `packages/client-core/src/generation/workflow.ts` | Retain |
