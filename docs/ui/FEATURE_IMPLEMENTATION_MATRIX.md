...
| Delete world version (guarded) | ADR 0015 | `DELETE /api/v1/worlds/:worldId/versions/:worldVersionId` | `deleteDialog` | Implemented and wired; **test coverage incomplete** | Live API | Backend checks 5 blocker categories; only 2 of 5 covered by integration tests per prior review (`docs/review/2026-07-30-codebase-review.md` §3, §5.2) | `server.ts:465-472`, `world-service.ts:795` | Retain UI; not a frontend gap, flag for backend test follow-up in `OPEN_QUESTIONS.md` |
...
| Provider `configuration` field redaction | AGENTS.md Security | **not implemented** — field returned unredacted | provider edit form | Implemented but incomplete (backend gap) | Live API | Prior review flags this as a Medium security finding | `docs/review/2026-07-30-codebase-review.md` §6 | Not a frontend fix; flag in `OPEN_QUESTIONS.md` so the replacement UI doesn't assume this field is safe to log/display carelessly |
