...
For Sogni, create a bearer API key in the [Sogni account dashboard](https://dashboard.sogni.ai/api-key), then create an **Illustrations** profile. Keep the official base URL unless a trusted deployment uses a documented alternative; Nexus appends the documented `/v1/creative-agent/workflows` and `/v1/models` paths. Leave the key field blank while editing to retain the encrypted credential; Nexus never repopulates it. See [Configure Sogni](../nexus-guide/providers/sogni.md) for image, retry, polling, and artifact-handling details.
