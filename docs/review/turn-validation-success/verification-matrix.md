...
### Recovery corrections and broad integration preparation

At immutable `62f09bdf`, coordinator verification passed **83/83 real PostgreSQL tests**, including the new immediate continuity-conflict provenance and explicit Retry followed by repair scenarios. The latter has one actual repair application; it must not be described as proof of two applications. The new long-prose planner regression at `6907d600` passed with its complete 56-test file: repaired narration equals the existing preview representation, all non-whitespace content is preserved, reparsing is idempotent, and the original raw bytes remain hash-bound.
