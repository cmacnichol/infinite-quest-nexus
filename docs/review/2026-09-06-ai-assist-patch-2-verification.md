...
P2.1–P2.10 passed their task-scoped reviews. The subsequent whole-branch review found four Important workflow gaps and three Minor defects. The consolidated final fix and its evidence are recorded below; independent scoped re-review and final adjudication remain pending. This record does not accept or publish Patch 2 and does not authorize Patch 3.
...
The whole-branch review at `eca1a7e042b292ae78713cbeb31386e5b4dff6cc` found four Important workflow gaps: untouched first world reviews were unsaved, standalone character detail omitted the generated candidate, pending replacement review lost selection intent, and ordinary character acceptance bypassed the durable handoff. It also found obsolete retry commands, a worker proof healthcheck problem, and an incorrect runbook capability URL. These findings superseded the earlier task acceptance as the overall completion gate.

The consolidated final fix passed its independent scoped re-review: all seven findings are addressed, with no new Critical or Important breakage. The controller accepted the fix after matching all 18 files to the reviewed snapshot and checking the retained verification evidence. The Patch 2 implementation gate is complete; publication is recorded separately in the pull request. No production deployment, live external model validation, or live Swarm test is claimed. Windows secure-staging skips remain skips. Git global-ignore access and line-ending advisories are retained environment warnings. Patch 3 was not started.

## Consolidated final fix verification

Explicit first review now saves untouched content with its current validated selection. Standalone character detail projects only the current validated, identity-checked checkpoint. Durable acceptance flushes the exact character review before returning to either parent. The existing editor restores original local parent edits and requires explicit receipt-backed apply; an unchanged parent stays saved. New-world parents remain local review drafts.

Selection recovery needs no database or wire-format migration. Historical selected stage IDs retain intent while any selected replacement is pending, failed, or recoverable. Review during that interval does not persist an empty selection or adopt new content. After validation, an explicit Review advances only those selected keys; cancelled obsolete roster keys can be removed explicitly without selecting new siblings. Polls and ordinary edits never advance generations. Deliberately empty saved selections remain empty. The cost is conservative blocking: a human edit made while its selection is stale cannot be saved until the replacement is explicitly reviewed; failed work must be retried or the author must choose another proposal. The old saved review survives reload, and conflict/unsaved states are not reported as successful saves.

New regressions live in the [session suite](../../tests/unit/web-next-authoring-job-session.test.ts), [character workspace suite](../../tests/unit/web-next-character-workspace-page.test.ts), [real HTTP/PostgreSQL suite](../../tests/integration/authoring-routes.integration.test.ts), [rendered suite](../../tests/e2e/ai-assist-resume.e2e.test.ts), and [worker healthcheck suite](../../tests/unit/authoring-worker-healthcheck.test.ts). Actual behavioral RED was retained for untouched review, pending selection loss, all four standalone create/edit target projections, premature character navigation, ordinary editor return blocking apply, both obsolete Retry controls, changed-roster selection recovery, and the inherited Swarm health reference. Harness setup failures remain separately recorded, including world-route composition/response-field mistakes, Chromium sandbox launch, Vite port reuse, a response interception race, and legacy browser fixtures missing the API's saved selection or assuming initial Review never saves.

The final focused UI run passed **238 tests in five files**. The affected real PostgreSQL run passed **75 tests in four files**, including repository fences, stage execution, route projection, and atomic apply. The worker configuration run passed **20 tests in two files**. The final rendered suite passed **29 tests**, including stale two-tab conflict/compare/reload, saved subsets, historical autosave acknowledgements, and frozen apply replay. Full repository/type checks, application build, documentation build, and diff checks passed. These overlap retained earlier evidence and are not an aggregate unique-test count.

The [real Compose final-fix proof](../../tests/fixtures/authoring/compose-final-fix-proof.mjs) uses the [public fixture](../../tests/fixtures/authoring/compose-p2-10.yaml) and [deterministic provider](../../tests/fixtures/authoring/compose-provider.mjs). It exercises an untouched world Generate → Review → Create flow, real API-requested pending outline replacement through browser reload, and eight character combinations: create/edit × normal/resumed × new/existing parent. New-parent normal cases seed the existing local session format from a real generated world; existing-parent cases launch through the rendered editor. All generation, review saves, recovery loads, authoritative applies, and receipts use real API/worker/PostgreSQL paths. Delayed review responses and dropped first apply responses wrap real server responses, without fabricated candidates or receipts. Four existing-parent cases verify exact frozen request replay, one draft revision increment, character identity, unselected roster preservation, and actual database receipts. New-parent cases verify local-only restoration and non-applicability. Browser page errors and unexpected console errors were empty; intentionally dropped responses produce expected network errors.

The worker-only proof fixture now uses database readiness. Live disposable health reports `healthy`, zero failing streak, and successful `SELECT 1` probes. This exposed a pre-existing reference error in the Swarm override: it called the nonexistent `databaseConfig()` export at the immutable branch base. The same narrow correction now uses `loadRuntimeConfig().databaseUrl` in both commands. The focused test executes each manifest command with real configuration resolution and controlled SQL success/failure. The corrected Swarm command passed against the disposable runtime, and the Swarm configuration rendered successfully. No Swarm service was deployed or tested live.

Screenshots are retained under the approved artifact root in `p2-final-fix/compose-complete` and `p2-final-fix/browser-complete`, at desktop and 390px widths. Existing sticky stage/save bars still overlap portions of long full-page captures; no horizontal overflow or framework error overlay was observed. The runtime build passed with the existing font-resolution and chunk-size warnings. Worker scheduling is unchanged, so the accepted C0 measurement remains applicable and was not repeated. The disposable project's original PostgreSQL container was attached to the public fixture's network without recreation; no existing deployment or persistent volume was changed.
