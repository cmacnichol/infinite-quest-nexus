...
Campaign narration length is an authoritative campaign default, not a provider limit. A Story submission may select one validated per-turn profile override. At enqueue time the server resolves the override or campaign default and snapshots the effective profile and word range into the durable generation job, so queued and retried work remains reproducible even if the campaign default or browser selection later changes. The provider maximum-output setting remains an independent hard ceiling used for context reservation and truncation detection.
