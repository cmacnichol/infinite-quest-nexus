...
The bridge trusts only completed Story Engine results. It does not maintain a second browser-side mechanics detector or provider-specific cleanup path; fiction-boundary validation, recovery, and acceptance are worker-owned so OpenRouter, LM Studio, Manifest, and compatible providers follow the same contract.
