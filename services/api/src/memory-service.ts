    ...
    `WITH base AS (
     ...
     SELECT id, turn_id, memory_kind, ordinal, content, token_estimate, importance, entities, entity_ids, relevance
      ...
      WHERE memory_kind IN ('campaign_summary','legacy_summary','open_thread')
         OR (memory_kind = 'canonical_fact' AND (recent_rank <= 64 OR ($3 <> '' AND lexical_rank <= 64) OR entity_ids && $6::text[]))
...
export async function buildContextPreview(
  ...
  const queryEntityIds = matchEntityReferences(options.query, entityCatalog).map((match) => match.entity.id);
  ...
  for (const [sourceTurnId, facts] of grouped) {
    ...
    await client.query(
      ...
      [ownerUserId, campaignId, worldVersionId, sourceTurnId, ordinal, content, estimateTokens(content), entities,
        [...new Set(facts.flatMap((fact) => fact.entity_ids))],
...
async function projectCanonicalFacts(
  ...
  const catalog = derived.entityCatalog ?? [];
  ...
  for (let index = 0; index < projections.length; index += 1) {
    ...
    const entityMetadata = resolveEntityMetadata(projection.content, catalog);
    await client.query(
      ...
      [projection.id, ownerUserId, campaignId, worldVersionId, turnId, ordinal, projection.factIndex,
        projection.content, canonicalFactDeduplicationKey(projection.content), entityMetadata.entities, entityMetadata.entityIds,
...
export async function storeDerivedTurnMemories(
  ...
  const scope = derived.entityCatalog ? null : await campaignScope(client, ownerUserId, campaignId);
  const entityCatalog = derived.entityCatalog ?? buildScopedEntityCatalog({
    worldContent: scope!.world_content,
    characterSnapshot: scope!.character_snapshot
  });
  derived.entityCatalog = entityCatalog;
  await projectCanonicalFacts(client, ownerUserId, campaignId, worldVersionId, turnId, ordinal, derived);
  if (summary) {
    const summaryEntities = resolveEntityMetadata(summary, entityCatalog);
    await client.query(
      `INSERT INTO chronicle_memories (
       ...
       ON CONFLICT (campaign_id, turn_id, memory_kind) DO UPDATE SET
         ...
         importance = EXCLUDED.importance, entities = EXCLUDED.entities, entity_ids = EXCLUDED.entity_ids, metadata = EXCLUDED.metadata,
    ...
    await client.query(
      `INSERT INTO chronicle_memories (
       ...
       ON CONFLICT (campaign_id, turn_id, memory_kind) DO UPDATE SET
         ...
         embedding_dimensions = NULL, embedding_content_hash = NULL, embedding_updated_at = NULL,
         embedding_provider_fingerprint = NULL, updated_at = now()`,
      [ownerUserId, campaignId, worldVersionId, ordinal, content, estimateTokens(content), threadEntities.entities, threadEntities.entityIds,
