...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-26 (commit d7f7d91). Confidence: 100%.
...
### Code health
Three co-equal signals: defect risk 5.09/10 avg, hotspot health 2.66/10 (stable), worst `services/api/src/generation-service.ts` at 1.0/10 · maintainability 6.99/10 · performance risk 34 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `packages/domain/src/world-characters.ts` — untested hotspot — impact −2.0
- `services/api/src/memory-service.ts` — untested hotspot — impact −2.0
- `services/api/src/provider-service.ts` — untested hotspot — impact −2.0
- `tests/integration/import-memory.integration.test.ts` — change entropy — impact −1.6
- `tests/integration/world-library.integration.test.ts` — large method (integration callback) — impact −1.5
