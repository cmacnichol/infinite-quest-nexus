...
Issues are tracked in GitHub Issues for `cmacnichol/infinite-quest-nexus`. See `docs/agents/issue-tracker.md`.
...
Use the multi-context layout, with a root context map and context-specific documentation under `apps/`, `services/`, and `packages/`. See `docs/agents/domain.md`.
...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-20 (commit 73b4f26). Confidence: 100%.
...
- `tests/integration/generation.integration.test.ts` — 12 bug fixes, last fix 8 days ago (bug magnet); 23 commits/90d
- `services/api/src/server.ts` — 12 bug fixes, last fix 3 days ago (bug magnet); 30 commits/90d
- `tests/integration/image-pipeline.integration.test.ts` — 11 bug fixes, last fix 11 days ago (bug magnet); 19 commits/90d
- `tests/unit/story-player-ui.test.ts` — 10 bug fixes, last fix today (bug magnet); 16 commits/90d
- `services/api/src/generation-service.ts` — 11 bug fixes, last fix 3 weeks ago (bug magnet); 47 commits/90d

### Code health
Three co-equal signals: defect risk 6.01/10 avg, hotspot health 4.35/10 (stable), worst `packages/application/src/assets/private-filesystem-repository.ts` at 1.0/10 · maintainability 7.83/10 · performance risk 154 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `packages/database/src/campaign-state-repository.ts` — change entropy — impact −3.0
- `tests/integration/campaign-authority-repository.integration.test.ts` — change entropy — impact −3.0
- `tests/unit/web-next-character-workspace-session.test.ts` — change entropy — impact −2.6
- `packages/database/src/chronicle-chunk-repository.ts` — complex conditional (validateProgress) — impact −2.5
- `docs/review/chronicle-retrieval-audit-future-enhancement.md` — churn risk — impact −2.4
