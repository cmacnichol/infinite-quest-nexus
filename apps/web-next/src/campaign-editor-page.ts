...
export function mountCampaignEditorPage(root: HTMLElement, route: CampaignRoute): MountedPage {
  let shell = mountAppShell(root, route.campaignId ? `<main id="main-content"><p class="campaign-loading">Loading campaign…</p></main>` : campaignListMarkup(), "campaigns"); const controller = new AbortController(); const storyMemoryApi = createStoryMemoryApi(); let disposed = false; let campaign: CampaignSummary | null = null; let storyMemorySettings: StoryMemorySettings | null = null; let storyMemoryUnavailable = false; let transferPreview: JsonRecord | null = null; let chronicleConfig: JsonRecord = {}; let chronicleProviders: ProviderSummary[] = []; let chronicleOperationActive = false; let stateBase: CampaignRuntimeStateResponse | null = null; let continuityEditor: CampaignContinuityEditor | null = null; let stateGenerationLocked = false; let stateConflictLocked = false; let stateReloadInFlight = false; let stateSaveInFlight = false; let stateSessionId = 0;
