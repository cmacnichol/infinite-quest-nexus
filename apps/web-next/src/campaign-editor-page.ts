...
function overviewMarkup(c: CampaignSummary, providers: ProviderSummary[], stateRevision: number): string {
...
export function mountCampaignEditorPage(root: HTMLElement, route: CampaignRoute): MountedPage {
  ...
  async function showSection(): Promise<void> {
    ...
    if(route.section==="overview") { const [providers, state] = await Promise.all([loadProviders(), campaignApi.get<CampaignRuntimeStateResponse>(campaign.id, "/state", controller.signal)]); if(disposed) return; target.innerHTML=overviewMarkup(campaign, providers, state.revision); }
