...
test("Story uses the saved profile and exposes no per-turn Model or Preset controls", async ({ page }) => {
  await page.addInitScript(() => {
    const originalFetch = window.fetch.bind(window);
    const signals: AbortSignal[] = [];
  ...
  await expect(page.locator("#turnTextSelectionPanel")).toHaveCount(0);
  await expect(page.getByRole("combobox", { name: "Text selection" })).toHaveCount(0);
  await page.locator("#freeAction").fill("Use the saved profile selection.");
  ...
  expect(writes[0]).not.toHaveProperty("textExecutionOverrides");
  await page.setViewportSize({ width: 390, height: 844 });
  await page.reload();
  await expect(page.locator("#turnTextSelectionPanel")).toHaveCount(0);
  expect(runtimeErrors).toEqual([]);
