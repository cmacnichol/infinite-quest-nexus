...
describe("preset route execution", () => {
  ...
  it("does not resend an already dispatched unknown attempt after reclaim", async () => {
    ...
    const prior = await repository.reserve({ logicalReservation: storyReservation, candidateOrdinal: 0, candidate: candidates[0]!, request: { body: "{}", payloadHash: "hash-0" } });
    await repository.markDispatched(storyReservation, prior!.id, "hash-0");
    const invoke = vi.fn();
    await expect(executePresetRoutes({
      candidates: candidates.slice(0, 1), logicalReservation: storyReservation, attempts: repository,
  ...
  it("rejects returned identities outside the frozen candidate and provider policy", async () => {
    await expect(executePresetRoutes({
      candidates: candidates.slice(0, 1), logicalReservation: storyReservation, attempts: attempts(),
      prepareCandidate: () => ({ body: "{}", payloadHash: "hash-0" }),
  ...
  it("bounds Retry-After by the total deadline", async () => {
    ...
    await expect(executePresetRoutes({
      candidates, logicalReservation: storyReservation, attempts: attempts(),
