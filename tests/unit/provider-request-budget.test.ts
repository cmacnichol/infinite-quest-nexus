...
describe("provider request serialization", () => {
  it.each(["openrouter", "lmstudio"] as const)("sends and audits the configured story output reserve for review through %s transport", async (providerType) => {
    const reviewProfile = { ...profile, providerType, contextWindowTokens: 80_000, maxOutputTokens: 32_000 };
    const sent: string[] = [];
    const fetcher = vi.fn(async (_url: unknown, init?: RequestInit) => {
      ...
      return new Response(JSON.stringify(providerType === "lmstudio"
        ? { response_id: "r", output: [{ type: "message", content: "{}" }], stats: { total_output_tokens: 32_000 } }
    ...
    const prepared = serializeCheckedProviderRequest(reviewProfile, request, { inputLimit: 80_000 - 32_000, count: (s) => Math.ceil(s.length / 3), output: request.budgetOutput });
    expect(prepared.budgetAudit?.outputReserveTokens).toBe(32_000);
    ...
    expect(JSON.parse(sent[0]!)[providerType === "lmstudio" ? "max_output_tokens" : "max_tokens"]).toBe(32_000);
  ...
  it("serializes a prepared strict OpenRouter contract into the exact dispatched body", async () => {
    ...
    const prepared = serializeProviderRequest(openRouter, {
      systemPrompt: "rules", input: "action", responseContract
    } as never);
    const sent: string[] = [];
    const fetcher = vi.fn(async (_url: string | URL | Request, init?: RequestInit) => {
      sent.push(String(init?.body));
      return new Response(JSON.stringify({ id: "actual", model: "actual/model", choices: [{ message: { content: "{}" }, finish_reason: "stop" }], usage: {} }), { status: 200 });
    });
