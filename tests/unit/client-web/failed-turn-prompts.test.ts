...
describe("failed append prompt storage", () => {
  it("keeps an append prompt available across recovery reloads until completion clears its campaign scope", () => {
    ...
    store.save({ campaignId: "campaign-a", expectedTurnNumber: 4, generationId: "job-a", requestedInputMode: "scene", action: "Return the lantern." });

    expect(store.load("campaign-a")).toEqual({ campaignId: "campaign-a", expectedTurnNumber: 4, generationId: "job-a", requestedInputMode: "scene", action: "Return the lantern." });
  ...
  it("treats inaccessible browser storage as unavailable without throwing", () => {
    ...
    expect(store.load("campaign-a")).toBeNull();
    expect(() => store.save({ campaignId: "campaign-a", expectedTurnNumber: 1, generationId: "job-a", requestedInputMode: "action", action: "Wait." })).not.toThrow();
