...
describe("createNexusHttpClient", () => {
  ...
  it("keeps the root base path API-relative", async () => {
    const queue = fetchQueue(jsonResponse({ value: "root" }));
    ...
    await expect(client.request({ method: "GET", path: "/worlds", responseSchema })).resolves.toEqual({ value: "root" });
