...
describe("scoped entity references", () => {
  it("tolerantly builds stable references from world entities and campaign character data", () => {
    const catalog = buildScopedEntityCatalog({
      ...
      characterProfile: {
        name: "Éowyn Vale",
        profile: { identity: { aliases: ["The Fox", "Shield Maiden"] } }
      }
    });
    ...
    expect(catalog).toEqual([
      ...
      { id: "character:hero-1", displayName: "Éowyn Vale", aliases: ["Éowyn Vale", "Shield Maiden", "The Fox"], kind: "character", source: "character" }
    ]);
  });
