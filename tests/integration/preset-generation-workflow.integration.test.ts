...
integration("durable preset physical attempts", () => {
  ...
  it("persists safe fallback attempts in frozen order and isolates logical owners", async () => {
    ...
    const execute = (logicalReservation: LogicalReservation, currentInvoke = invoke) => executePresetRoutes({
      candidates, logicalReservation, attempts,
    ...
    const rows = await pool.query<{ reservation_key: string; candidate_ordinal: number; outcome: string; usage: unknown; reported_cost: unknown }>(
      `SELECT reservation_key,candidate_ordinal,outcome,usage,reported_cost
    ...
    expect(rows.rows.filter((row) => row.reservation_key.startsWith(firstScope.requestScopeId))).toEqual([
      expect.objectContaining({ candidate_ordinal: 0, outcome: "failed", usage: null, reported_cost: null }),
      expect.objectContaining({ candidate_ordinal: 1, outcome: "succeeded", usage: null, reported_cost: null })
    ]);
  ...
  it("rejects late completion after durable lease loss and records no accounting", async () => {
    ...
    await expect(executePresetRoutes({
      candidates: candidates.slice(0, 1), logicalReservation: fixture.reservation, attempts,
  ...
  it("never resends or advances an unknown dispatched attempt after reclaim", async () => {
    ...
    const reserved = await attempts.reserve({ logicalReservation: fixture.reservation, candidateOrdinal: 0, candidate: candidates[0]!, request });
    ...
    const invoke = vi.fn();
    await expect(executePresetRoutes({
      candidates, logicalReservation: reclaimed, attempts,
