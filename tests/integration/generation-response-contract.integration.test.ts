...
integration("PostgreSQL response-contract persistence", () => {
  ...
  it("binds multiple concrete operations to the persisted logical attempt and finalizes each response once", async () => {
    ...
    await expect(repository.reserveResponseContractInvocation!(scope, { logicalAttemptId: logicalAttemptId!, invocationKey: "story:nonstream", operation: "story_generation", requestPayloadHash: hash, request: { ...audit, requestedModel: "tampered-model" } })).resolves.toBeNull();
