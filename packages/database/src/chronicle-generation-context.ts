...
function completeRules(value: unknown): readonly string[] {
  if (typeof value !== "string" || !value.trim() || value.length > 20_000 || /[\u0000-\u001f\u007f]/u.test(value)) {
    return invalidRules();
  }
  return [value];
}
...
export async function loadPostgresChronicleGenerationContext(
  ...
  const campaign = await client.query<{ mandatory_rules: unknown; selected_character_id: string | null }>(
    `SELECT wv.content->'world'->>'rules' AS mandatory_rules, c.selected_character_id
       FROM campaigns c
  ...
  const candidates = await client.query<{
    ...
    `SELECT id, turn_id, ordinal, memory_kind, content, token_estimate
      ...
      WHERE owner_user_id = $1 AND campaign_id = $2 AND world_version_id = $3
        AND ordinal <= $4
      ORDER BY ordinal DESC, importance DESC, id
      LIMIT 512`,
    [scope.ownerUserId, scope.campaignId, scope.worldVersionId, baseTurnNumber]
  );
  const continuity = currentContinuity === null ? null : currentContinuitySchema.parse(currentContinuity);
  return {
    authority: {
      rules: completeRules(campaignRow.mandatory_rules),
