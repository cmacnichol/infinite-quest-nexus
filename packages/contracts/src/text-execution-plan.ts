...
export const textExecutionPlanSchema = z.object({
  version: z.literal(2), selection: textModelSelectionSchema, preset: z.object({ slug: z.string().trim().min(1).max(200), versionId: z.string().trim().min(1).max(500), configHash: hashSchema }).strict().nullable(), candidates: z.array(textRouteCandidateSchema).min(1).max(32), presetSystemPrompt: z.string().max(200_000), parameters: textGenerationParametersSchema, prompt: z.string().min(1).max(400_000), promptHash: hashSchema, endpointReference: z.string().trim().min(1).max(500), credentialReference: z.string().trim().min(1).max(500).nullable(), profileRevision: z.string().trim().min(1).max(500), protocolVersion: z.string().trim().min(1).max(500), planHash: hashSchema
}).strict();
