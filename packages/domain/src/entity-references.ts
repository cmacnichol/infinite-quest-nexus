...
function fallbackId(source: EntityReferenceSource, kind: string, displayName: string): string {
  const normalized = normalizeEntityTerm(displayName);
...
function toReference(value: unknown, source: EntityReferenceSource, defaultKind: string): EntityReference | undefined {
  ...
  const candidates = [displayName, ...textValues(record.aliases), ...textValues(record.alias)];
...
function worldEntityValues(worldContent: unknown): unknown[] {
  const content = asRecord(worldContent);
...
/**
 * Builds references only from the supplied world version and campaign character
 * snapshot. It deliberately performs no global lookup, preserving retrieval scope.
 */
...
export function buildScopedEntityCatalog(
  ...
  const isInput = suppliedCharacterSnapshot === undefined
    ...
    && (Object.hasOwn(possibleInput!, "worldContent") || Object.hasOwn(possibleInput!, "characterSnapshot"));
  ...
  const character = toReference(characterSnapshot, "character", "character");
  ...
  for (const candidate of candidates) {
    ...
    if (!byScopedId.has(key)) byScopedId.set(key, candidate);
  }
  return [...byScopedId.values()];
}

type AliasCandidate = { alias: string; normalized: string; entity: EntityReference };
...
function unambiguousAliases(catalog: readonly EntityReference[]): AliasCandidate[] {
  ...
  for (const entity of catalog) {
    for (const alias of entity.aliases) {
      ...
      if (!entries.some((entry) => entry.entity.id === entity.id && entry.entity.source === entity.source)) {
...
export function matchEntityReferences(text: string, catalog: readonly EntityReference[]): EntityReferenceMatch[] {
  ...
  for (const candidate of unambiguousAliases(catalog)) {
    for (const match of normalizedText.matchAll(phrasePattern(candidate.normalized))) {
      ...
      const entityKey = `${candidate.entity.source}:${candidate.entity.id}`;
      if (!matchedEntities.has(entityKey)) {
        matches.push({ entity: candidate.entity, matchedAlias: candidate.alias });
        matchedEntities.add(entityKey);
      }
...
export function findEntityReferences(text: string, catalog: readonly EntityReference[]): EntityReference[] {
...
export function entityQueryTerms(query: string, catalog: readonly EntityReference[]): string[] {
  const seen = new Set<string>();
  const terms: string[] = [];
...
export function expandEntityQuery(query: string, catalog: readonly EntityReference[]): string {
  const value = String(query ?? "").trim();
  const original = normalizeEntityTerm(value);
  const additions = entityQueryTerms(value, catalog).filter((term) => normalizeEntityTerm(term) !== original);
  return [value, ...additions].filter(Boolean).join(" ");
}

/** Heuristic discoveries are separate so callers never confuse them with stable IDs. */
export function extractCapitalizationFallback(text: string, catalog: readonly EntityReference[] = [], limit = 32): string[] {
  const known = new Set(catalog.flatMap((entity) => entity.aliases.map(normalizeEntityTerm)));
  return extractEntities(text, limit).filter((value) => !known.has(normalizeEntityTerm(value)));
}

export function resolveEntityMetadata(text: string, catalog: readonly EntityReference[]) {
  ...
  return {
    ...
    entities: [...new Set([...displayNames, ...extractCapitalizationFallback(text, catalog)])].slice(0, 32)
  };
}
