...
export function buildSourceExtractionPrompt(input: Readonly<{
  instructions: string;
  sourceText: string;
...
export function buildSourceWorldPrompt(input: Readonly<{
  ...
  const expansionContract = input.selection.mode === "expand"
    ? "Expansion mode may additionally return separately labeled expansionCandidates. Each is {\"target\":\"world\" or a selected identity representative,\"path\":\"closed mapped field path\",\"value\":\"proposed value\",\"supportingFactIds\":[\"current reviewed fact id\"]}. target must be world or a supplied selected identity representative; supporting IDs for an identity target must belong to that identity group. These are invented review candidates only, never canonical fields, so proposed values need not copy a reviewed value. Do not propose mechanics, stats, or trackers."
    : "";
