...
export const generationReviewCheckpointSchema = z.strictObject({
  ...
  for (const candidate of [checkpoint.originalCandidate, checkpoint.gateCandidate, checkpoint.workingCandidate]) {
    if (candidate.ownerUserId !== binding.ownerUserId || candidate.campaignId !== binding.campaignId || candidate.worldId !== binding.worldId
      ...
      || candidate.expectedTurnNumber !== binding.expectedTurnNumber || candidate.policyHash !== binding.policyHash || candidate.protocol.version !== binding.protocol.version
  ...
  for (const entry of checkpoint.decisionJournal) {
    if (entry.candidateScope !== checkpoint.candidateScope || entry.candidateHash !== checkpoint.gateCandidate.storyHash) {
      context.addIssue({ code: "custom", path: ["decisionJournal"], message: "Decision evidence must bind the offered candidate." });
      break;
    }
  }
});
